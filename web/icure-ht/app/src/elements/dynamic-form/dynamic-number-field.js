class DynamicNumberField extends Polymer.TkLocalizerMixin(Polymer.mixinBehaviors([], Polymer.Element)) {
  static get template() {
    return Polymer.html`
        <style>
            :host {
  ...
  _inputValueChanged(value) {
    if (this.inputValue !== '' + this.value) {
      ...
      if (!this.inputValue.match(/^ *([+-]?[0-9]+(?:[.,]0*)) *$/) /*intermediate situation*/) {
        this.set('value', match && (match[1] ? parseFloat(match[1].replace(/([0-9]),([0-9])/, "$1.$2")) : null) || (isNaN(parseFloat(this.inputValue)) ? null : parseFloat(this.inputValue)))

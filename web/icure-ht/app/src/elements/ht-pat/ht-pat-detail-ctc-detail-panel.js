...
class HtPatDetailCtcDetailPanel extends Polymer.TkLocalizerMixin(Polymer.mixinBehaviors([], Polymer.Element)) {
  static get template() {
    return Polymer.html`
        <style>
            .details-panel {
        ...
        <div class="details-panel" on-dragover="_onDrag">
            <div class="layout horizontal">
                <filter-panel id="serviceFilterPanel" items="[[detailPanelItems]]"></filter-panel>
                <div class="layout-bar">
                    <template is="dom-if" if="[[!list]]">
                        <paper-icon-button id="list_icn" class="list" icon="icons:view-agenda" on-tap="_list"></paper-icon-button>
                    </template>
                    <template is="dom-if" if="[[!table]]">
                        <paper-icon-button id="table_icn" class="table" icon="icons:view-list" on-tap="_table"></paper-icon-button>
                    </template>
                    <template is="dom-if" if="[[!graphique]]">
                        <paper-icon-button id="graph_icn" class="graphique" icon="editor:pie-chart" on-tap="_graphique"></paper-icon-button>
                    </template>
                    <template is="dom-if" if="[[!doc]]">
                        <paper-icon-button id="archive_icn" class="doc" icon="icons:archive" on-tap="_default"></paper-icon-button>
                    </template>
                    ...
                    <paper-tooltip for="graph_icn" position="bottom" animation-delay="0">[[localize('gra','Graph',language)]]</paper-tooltip>
                    <paper-tooltip for="archive_icn" position="bottom" animation-delay="0">[[localize('arc','Archive',language)]]</paper-tooltip>
                </div>
            </div>
                    ...
                    <template is="dom-repeat" items="[[contactsFormsAndDocuments]]" as="dof">
                        ...
                        <template id="formsRepeat" is="dom-repeat" items="[[dof.forms]]" as="form">
                            <template is="dom-if" if="[[form.id]]">
                                <dynamically-loaded-form id="dynamicallyLoadedForm" api="[[api]]" user="[[user]]" patient="[[patient]]" form-id="[[form.id]]" contact="[[dof.ctc]]" health-elements="[[healthElements]]" contacts="[[contacts]]" services-map="[[servicesMap]]" i18n="[[i18n]]" language="[[language]]" resources="[[resources]]" on-edit-form="edit" on-form-deleted="formDeleted" on-new-service="_newService"></dynamically-loaded-form>
                            </template>
                            <template is="dom-repeat" items="[[form.docs]]">
                                <dynamic-doc api="[[api]]" user="[[user]]" service="[[item]]" i18n="[[i18n]]" language="[[language]]" resources="[[resources]]" title="[[item.label]]"></dynamic-doc>
                            </template>
                        </template>
                    </template>
                </template>
            ...
            <div class="floating-action-bar">
                <div id="prescribeBtnCtnr">
                    <paper-button id="prescribeBtn" class="prescribe-btn" on-tap="_prescribe" disabled="[[!_canPrescribe(api.tokenId,patient.ssin,servicesMap.*)]]">
                ...
                <template is="dom-if" if="[[_hasCurrentContact(contacts)]]">
                    <template is="dom-if" if="[[!showAddFormsContainer]]">
                        <paper-button class="add-forms-btn" on-tap="_toggleAddActions">
                            <iron-icon icon="[[_actionIcon(showAddFormsContainer)]]"></iron-icon>
                            [[localize('add_for','Add forms',language)]]
                        </paper-button>
                    </template>
                    <template is="dom-if" if="[[showAddFormsContainer]]">
                        <paper-button class="close-add-forms-btn" on-tap="_toggleAddActions">
                            <iron-icon icon="[[_actionIcon(showAddFormsContainer)]]"></iron-icon>
                            [[localize('clo','Close',language)]]
                        </paper-button>
                    </template>
                    <div class="add-forms-container" hidden\$="[[!showAddFormsContainer]]">
                        <paper-fab-speed-dial-action icon="vaadin:doctor-briefcase" on-tap="addConsultation">[[localize('con','Consultation',language)]]
                        </paper-fab-speed-dial-action>
                        <paper-fab-speed-dial-action icon="vaadin:chart-line" on-tap="addMedicalHistory">[[localize('med_his','Medical history',language)]]
                        </paper-fab-speed-dial-action>
            ...
            <paper-tooltip for="prescribeBtnCtnr" position="top">
                <template is="dom-if" if="[[_canPrescribe(api.tokenId,patient.ssin,servicesMap.*)]]">[[localize('pre_thi_but_to_pre','Press this button to
                    prescribe',language)]]<br></template>
                <template is="dom-if" if="[[!api.tokenId]]">[[localize('you_mus_be_con_to_ehe_to_be_all_to_pre','You must be connected to eHealth to be allowed to
                    prescribe',language)]]<br></template>
                <template is="dom-if" if="[[!_validSsin(patient.ssin)]]">The niss of the patient is not valid or missing ([[patient.ssin]])<br></template>
                <template is="dom-if" if="[[!_hasDrugsToBePrescribed(servicesMap.*)]]">[[localize('add_pre_dru_to_cur_con_to_pre','Add prescription drugs to current contact to
                    prescribe',language)]]
                </template>
            </paper-tooltip>
        </div>
        ...
        <paper-dialog id="upload-dialog">
            ...
            <vaadin-upload id="vaadin-upload" no-auto="" files="{{files}}" accept="video/*,image/*,application/pdf" target="[[api.host]]/document/{documentId}/attachment/multipart" method="PUT" form-data-name="attachment" on-upload-success="_fileUploaded"></vaadin-upload>
        </paper-dialog>

        <paper-dialog id="prescriptionDialog">
            <h2 class="modal-title">[[localize('pri_pre','Print prescription',language)]]</h2>
            <vaadin-grid id="entities-list" class="material" items="[[_drugsToBePrescribed(currentContact.services.*,_drugsOnPrescriptions,_drugsOnPrescriptions.*,servicesMap.*.content)]]" active-item="{{activeDrug}}">
...
`;
      ...
      detailPanelItems: {
        ...
        value: function () {
          return [{icon: "icure-svg-icons:laboratory", title: "Lab Results", id: "LabResults"}, {
            ...
            title: "Prescription",
            id: "Prescription"
          }, {icon: "icure-svg-icons:blood-sugar", title: "Blood Sugar", id: "BloodSugar"}, {
            icon: "icure-svg-icons:blood-pressure",
            title: "Blood Pressure",
            id: "BloodPressure"
          }, {icon: "icure-svg-icons:prescription-drugs", title: "Prescription Drugs", id: "PrescriptionDrugs"}]
        }
      },
  ...
  _printPrescriptions(e) {
    if (this.patient.ssin && this.api.tokenId) {
      ...
      this.api.hcparty().getHealthcareParty(this.user.healthcarePartyId).then(hcp => Promise.all(splitColumns.map(c => this.api.fhc().Recipecontroller().createPrescriptionUsingPOST(this.api.keystoreId, this.api.tokenId, "persphysician", hcp.nihii, hcp.ssin, hcp.lastName, this.api.credentials.ehpassword, {
        ...
        pdf.save(`presc_${this.patient.lastName || 'Doe'}_${this.patient.lastName || 'John'}_${this.patient.dateOfBirth || '00000000'}.pdf`)
      }))
    }
  }
  ...
  edit(e, form) {
    if (!this.currentContact.subContacts.find(sc => sc.formId === form.id)) {
      this.currentContact.subContacts.push({formId: form.id, services: [], healthElementId: form.healthElementId, planOfActionId: form.planOfActionId})
    }
    this.dispatchEvent(new CustomEvent('select-current-contact', {detail: form, composed: true}))
  }
  ...
  _contactsChanged() {
    ...
    const formGroups = this.contacts.map(contact => ({
      ctc: contact, forms: _.sortBy((contact.subContacts || []).map(sc => {
    ...
    climbFormHierarchy(_.uniq(_.flatten(formGroups.map(fg => fg.forms.filter(f => f.id).map(f => f.id)))), {}).then(() => {
      formGroups.forEach(fg => {
        fg.forms = fg.forms.reduce((acc, form) => {
  ...
  formTemplatesSelectorDataProvider() {
    return {
      filter: function (filterValue, limit, offset, sortKey, descending) {
        ...
        return this.api.form().findFormTemplates().then(fts => {

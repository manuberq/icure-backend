...
class HtPatHeTreeDetail extends Polymer.TkLocalizerMixin(Polymer.mixinBehaviors([], Polymer.Element)) {
  static get template() {
    return Polymer.html`
...
`;
  ...
  select(e) {
    ...
    if (e.detail.sourceEvent.metaKey) {
      this.dispatchEvent(new CustomEvent('selection-change', {detail: {selections: [{action: 'select', items: [this.getHeId(this.he)]}]}}))
    } else {

class IccFormXApi extends Polymer.mixinBehaviors([], Polymer.Element) {
  static get template() {
    return Polymer.html`
        <style>
        </style>
`;
  ...
  newInstance(user, patient, c) {
    ...
    return this.crypto.extractDelegationsSFKs(patient, user.healthcarePartyId).then(secretForeignKeys => this.crypto.initObjectDelegations(form, patient, user.healthcarePartyId, secretForeignKeys[0])).then(initData => {
      ...
      (user.autoDelegations ? (user.autoDelegations.all || []).concat(user.autoDelegations.medicalInformation || []) : []).forEach(delegateId => promise = promise.then(form => this.crypto.appendObjectDelegations(form, patient, user.healthcarePartyId, delegateId, initData.secretId)).then(extraData => _.extend(form, {

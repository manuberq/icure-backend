...
$_documentContainer.innerHTML = `<dom-module id="icc-form-x-api">
    <template>
        <style>
        </style>
    </template>
...
class IccDocumentXApi extends Polymer.mixinBehaviors([], Polymer.Element) {
  static get is() {
    return 'icc-document-x-api'
  }
  ...
  init() {
    this.baseApi = this.api.document()
  ...
  newInstance(user, patient, c) {
    ...
    return patient ? this.crypto.extractDelegationsSFKs(patient, user.healthcarePartyId).then(secretForeignKeys => this.crypto.initObjectDelegations(document, patient, user.healthcarePartyId, secretForeignKeys[0])).then(initData => {
      ...
      (user.autoDelegations ? (user.autoDelegations.all || []).concat(user.autoDelegations.medicalInformation || []) : []).forEach(delegateId => promise = promise.then(document => this.crypto.appendObjectDelegations(document, patient, user.healthcarePartyId, delegateId, initData.secretId)).then(extraData => _.extend(document, {
  ...
  findBy(hcpartyId, patient) {
    return this.crypto.extractDelegationsSFKs(patient, hcpartyId).then(secretForeignKeys => this.api.document().findByHCPartyPatientSecretFKeys(hcpartyId, secretForeignKeys.join(','))).then(documents => this.decrypt(hcpartyId, documents)).then(function (decryptedForms) {
  ...
  getAttachmentUrl(documentId, attachmentId, sfks) {
    return url = this.baseApi.host + "/document/{documentId}/attachment/{attachmentId}".replace("{documentId}", documentId).replace("{attachmentId}", attachmentId) + (sfks && sfks.length ? '?sfks=' + sfks.join(',') : '')
  }
...
customElements.define(IccDocumentXApi.is, IccDocumentXApi)
